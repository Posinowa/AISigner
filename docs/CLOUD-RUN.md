...
⚠️ **ADC'ye düşmek yavaş DEĞİL — ölçüldü (403 ms).** Bir ara sürümde "GCP
dışında ADC keşfi asılır" varsayımıyla bir `K_SERVICE` kapısı eklenmişti;
varsayım yanlış çıktı. Kimlik yokken SDK metadata sunucusunu yokluyor ama
hızlıca vazgeçip `Could not load the default credentials` fırlatıyor — yani
#335'in "hemen fırlat, çağıran mock'a düşsün" sözleşmesi korunuyor. Kapı
kaldırıldı; gereksiz olmasının yanında Cloud Run **dışındaki** GCP
ortamlarını (GCE/GKE) da dışlıyordu.
